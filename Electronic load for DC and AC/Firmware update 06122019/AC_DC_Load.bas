...
'AC/DC switch omitted ==> always AC/DC
'LOad mode R const/I const selectable
...
Config Timer0 = Timer , Prescale = 256                      '16MHz /256 =    16usec
On Ovf0 Tim0_isr

Config Timer2 = Timer , Prescale = 1024                     '16MHz/1024 = 64usec
On Ovf2 Tim2_isr
...
Dim Dest_resistance As Single
Dim Min_resistance As Single
Dim Dest_resistance_range As Single
Dim Dest_current As Single
Dim Max_current As Single
Dim Dest_current_range As Single
Dim Temp_current As Single
Dim Max_temp_value As Word

Dim Load_mode As Byte
...
Do
   ...
   'Check max. Destination current and min destination resistor when load relay = Off
   'Imax = Pmax/Vrms
   'Rmin = Vrms/Imax
   If On_off_relay = 0 Then
      ...
      End If
      Min_resistance = Vrms / Dest_current_range
      Min_resistance = Min_resistance * 1000                'V/mA = kOhms ==> calculate Ohms
      Dest_resistance_range = Min_resistance * 100
   End If
   ...
   If Counter_1 >= Msec_500 Then                            'Read switches, calculate heatsink temperature and update display every 500msec
      ...
      If Remote = 0 Then
         If Load_mode_switch = 0 Then                       'R const
            Load_mode = R_const
         Else
            Load_mode = I_const
         End If
   ...
   If Remote = 0 Then                                       'Read destination current pot in local mode
      If Dest_value <> Dest_value_bak Then
         If Load_mode = I_const Then                        'I const mode
            Dest_current = Dest_value / 1023
            Dest_current = Dest_current * Dest_current_range
            Dest_value_bak = Dest_value
         Else                                               'R const
            Dest_resistance = Dest_value / 1023
            Dest_resistance = Dest_resistance * Dest_resistance_range
            Dest_resistance = Dest_resistance_range - Dest_resistance       'Imin = Rmax
            Dest_resistance = Dest_resistance + Min_resistance
            Dest_value_bak = Dest_value
         End If
         Disable Timer0
   ...
   While Ischarwaiting() > 0                                'Check if characters available on COM port
      ...
      If Cin = Lf Then
         ...
         End If
         Select Case Cmd
            Case "A" : Print "AAC DC Load V2.0"             'Send ID string
            ...
            Case "F":                                       'Set Dest current
               If Remote = 1 Then
               Max_temp_value = Dest_current_range
                  If Val(arg_string) < Max_temp_value Then
            ...
            Case "H":                                       'Print Heatsink temperature
               Print "H" ; Temp
            Case "I":                                       'Switch Load mode in Remote Mode
               If Remote = 1 Then
                  If Arg_string = "1" Then
                       Load_mode = R_const
                  Else
            ...
            Case "U":                                       'send u(i) and i(i) only for testing purposes
               Vai_request = 1
            Case "V":
               Print "V" ; Min_resistance                   'send minimum resistor in Ohms
            Case "W":
               Print "W" ; Dest_resistance_range            'send destination resistor range
            Case "X":                                       'Set destination resistor (in Ohms ) when remote = 1 and value >= minimum resistor
               If Remote = 1 Then
               Max_temp_value = Min_resistance
                  If Val(arg_string) >= Max_temp_value Then
                     Dest_resistance = Val(arg_string)
                     Disable Timer0
                     Overcurrent_wait_counter = Overcurrent_wait_time
                     Enable Timer0
                  End If
               End If
            Case "Y":
               Print "Y" ; Dest_resistance                  'Send destination resistor in Ohms
         End Select
         Cmd_string = ""
      Else
         If Cin <> Cr Then
...
'Calculate DAC value for destination current setting in I const mode or DAC value for destination resistor
'in R-const mode
...
Sub Set_dest_current()
   ...
   Else
      If Load_mode = R_const Then                           'Dest_current = Vrms/Dest_resistance
         Dest_current = Vrms / Dest_resistance
         Dest_current = Dest_current * 1000                 'convert to mA
      End If
